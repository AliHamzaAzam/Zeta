// NOTE: Do verify input and output filenames in main function before execution
...
public:
                ...
                while (localChanged && currentProductions.size() > 1) {
                    ...
                    for (size_t i = 0; i < currentProductions.size(); ++i) {
                        ...
                        if (!longestPrefixTokens.empty() && !commonGroupIndices.empty()) {
                            ...
                            if (suffix_i_str.empty()) suffix_i_str = "#"; // Use epsilon if suffix is empty
                            ...
                            for (size_t idx : commonGroupIndices) {
                                ...
                                if (suffix_idx_str.empty()) suffix_idx_str = "#";
    ...
    int leftRecursion() {
        ...
        for (const auto& rule : cfg) {
            ...
            if (nonLeftRecursiveProds.empty()) { nonLeftRecursiveProds.emplace_back("#"); }
            ...
            for (const string& beta : nonLeftRecursiveProds) {
                ...
                if (newProd != "#") { newProd += " " + newNonTerminal; }
                else { newProd = newNonTerminal; }

                newLhsProds.push_back(newProd);
            }

            // Create productions for A' -> α A' | #
        ...
        for (const auto& rule : cfg) {
            for (const string& prod : rule.second) {
                ...
                while (iss >> token) {
                    ...
                    if (token != "#" && nonTerminals.find(token) == nonTerminals.end()) { terminals.insert(token); }
                }
            }
        }
    }
        ...
        while (changed) {
            ...
            for (const auto& rule : cfg) {
                ...
                for (const string& prodStr : rule.second) {
                    ...
                    // Handle direct epsilon production: X -> #
                    if (prod.empty() || (prod.size() == 1 && prod[0] == "#")) {
                    ...
                    // flag to track if all symbols in production can derive #
                    ...
                    for (const auto& symbol : prod) {
                        ...
                        for (const string& elem : first[symbol]) {
                            if (elem == "#") currDerivedEpsilon = true;
                            else {
    ...
    bool computeFirstOfSequence(const vector<string>& symbols, set<string>& firstSet) {
        ...
        if (symbols.empty() || (symbols.size() == 1 && symbols[0] == "#")) {
            firstSet.insert("#");
            ...
            for (const auto& rule : cfg) {
                ...
                for (const string& prodStr : rule.second) {
                    ...
                    for (size_t i = 0; i < prod.size(); ++i) {
                        ...
                        // Rule 3: A -> α B or A -> α B β where First(β) contains #
    ...
    int computeParsingTable() {
        ...
        for (const auto& rule : cfg) {
            ...
            for (const string& prodStr : rule.second) { // α
                ...
                // Rule 2: If # is in FIRST(α), then for each terminal 'b' in FOLLOW(A), add A -> α to M[A, b]
                if (alphaDerivesEpsilon) { // Check if the whole sequence α can derive #
        ...
        for (const auto& nonTerm : nonTerminals) {
            ...
            for (const auto& term : terminals) {
                ...
                if (parsingTable.find(key) != parsingTable.end()) {
                    ...
                    // Replace # with ^
                    size_t pos;
                    while ((pos = production.find("#")) != string::npos) production.replace(pos, 2, "^");
                    */
                }
...
int main() {
    ...
    if (cfg.readGrammar(fileName)) {
        ...
        cfg.writeParsingTableToCSV("ll1_parsing_table1.csv");
